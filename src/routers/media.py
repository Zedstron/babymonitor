...
def create_router(sio):
    ...
    async def play_media(request: Request, index: int):
        ...
        lullabies = request.app.state.appstate.media.getlist()
        if 0 <= index < len(lullabies):
            update = request.app.state.appstate.media.play(index, event)
    ...
    async def download_media(request: Request, index: int):
        path = request.app.state.appstate.media.read(index)
    ...
    async def pause_media(request: Request, index: int):
        lullabies = request.app.state.appstate.media.getlist()

        if 0 <= index < len(lullabies):
            request.app.state.appstate.media.pause(index)
    ...
    async def stop_media(request: Request, index: int):
        lullabies = request.app.state.appstate.media.getlist()

        if 0 <= index < len(lullabies):
            request.app.state.appstate.media.stop(index)
    ...
    @router.post("/upload")
    async def upload_media(request: Request, file: bytes = File(...), filename: str = Form(...)):
        if not filename.lower().endswith(('.mp3', '.wav', '.ogg')):
            raise HTTPException(status_code=400, detail="Unsupported file type")

        request.app.state.appstate.media.upload(file, filename)

        return { "status": True, "message": f"Uploaded {filename}" }

    @router.delete("/{index}")
    async def delete_media(request: Request, index: int):
        lullabies = request.app.state.appstate.media.getlist()

        if 0 <= index < len(lullabies):
            request.app.state.appstate.media.delete(index)
            return { "status": True, "message": f"Deleted {lullabies[index]}" }

        raise HTTPException(status_code=404, detail="Media not found")

    @router.get("/volume/{value}")
    async def set_track_volume(request: Request, value: int):
        if 0 <= value <= 100:
            request.app.state.appstate.media.volume(value)
    ...
    async def set_track_position(request: Request, value: int):
        if 0 <= value <= 100:
            request.app.state.appstate.media.seek(value)
    ...
    async def get_media(request: Request, ):
        return { "lullabies": request.app.state.appstate.media.getlist() }

    @router.get("/status")
    async def get_media_status(request: Request, ):
        return request.app.state.appstate.media.get_current()

    @router.get("/mute/on")
    async def set_mute_on(request: Request, ):
        request.app.state.appstate.media.mute(True)
        return { "status": True, "message": "Successfully Updated" }

    @router.get("/mute/off")
    async def set_mute_off(request: Request, ):
        request.app.state.appstate.media.mute(False)
        return { "status": True, "message": "Successfully Updated" }

    @router.get("/loop/on")
    async def set_loop_on(request: Request, ):
        request.app.state.appstate.media.loop(True)
        return { "status": True, "message": "Successfully Updated" }

    @router.get("/loop/off")
    async def set_loop_off(request: Request, ):
        request.app.state.appstate.media.loop(False)
        return { "status": True, "message": "Successfully Updated" }
